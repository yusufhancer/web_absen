const SIHADIR_API_BASE = (() => {
  const marker = "/web-absen/";
  const index = location.pathname.indexOf(marker);
  if (index >= 0) return `${location.origin}${location.pathname.slice(0, index + marker.length)}api`;
  return "../api";
})();
...
async function sihadirFetch(path, options = {}) {
  ...
  try {
    ...
    throw new Error(`API bukan JSON (${response.status}). Cek URL: ${SIHADIR_API_BASE}${path}`);
  }
